...
export async function initMap(
  ...
  if (!container || !document.body.contains(container)) {
    console.error('[map-view] container ไม่อยู่ใน DOM');
    return null;
  }
  ...
  try {
    ...
    console.error('[map-view] สร้างแผนที่ไม่ได้:', err.message);
    container.innerHTML = `<div style="height:100%;display:flex;align-items:center;justify-content:center;background:#f1f5f9;flex-direction:column;gap:8px;color:#64748b;font-size:14px">
      <span style="font-size:32px">🗺️</span>
      <span>Google Maps API Key ไม่ถูกต้อง</span>
      <span style="font-size:11px">ตรวจสอบ Console สำหรับรายละเอียด</span>
    </div>`;
    return null;
  }
}
 ...
 * คืน object ที่มี clearLayers() เพื่อ reset markers (compatible กับ code เดิม)
 */
...
export function addUserMarker(map, lat, lng) {
  if (!map) return { setMap() {}, getPosition() { return { lat: () => lat, lng: () => lng }; } };
